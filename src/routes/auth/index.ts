  ...
  .post('/login', zValidator('json', zLoginSchema), async (c) => {
    try {
      ...
      const user = await prisma.user.findFirst({
        where: {
          OR: [{ email: data.email }, { documentNumber: data.email }],
        },
      });
  ...
  .post('/register', zValidator('json', zRegisterSchema), async (c) => {
    try {
      ...
      const existingUser = await prisma.user.findFirst({
        where: {
          OR: [{ email: data.email }, { documentNumber: data.documentNumber }],
        },
      });

      if (existingUser) {
        return c.json({ error: 'Email or document number already registered' }, 409);
      }
